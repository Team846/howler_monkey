...
IntakeSubsystem::IntakeSubsystem(bool init)
    ...
    note_detection{intake_esc_.esc_.GetForwardLimitSwitch(rev::SparkLimitSwitch::Type::kNormallyOpen)},
    note_detection_other{intake_esc_.esc_.GetReverseLimitSwitch(rev::SparkLimitSwitch::Type::kNormallyOpen)} {
    if (init) {
...
IntakeTarget IntakeSubsystem::ZeroTarget() const {
  IntakeTarget target;
  target.target_state = kIntakeIdle;
  return target;
}
...
IntakeReadings IntakeSubsystem::GetNewReadings() {
  ...
  frc846::util::ShareTables::SetBoolean("intake_has_piece", note_detection.Get());
  readings_has_piece_graph.Graph(has_piece_);

  readings_intake_speed_.Graph(intake_esc_.GetVelocity().to<double>());
...
void IntakeSubsystem::DirectWrite(IntakeTarget target) {
  if (target.target_state == kIntake) {
    ...
    frc846::util::ShareTables::SetBoolean("intake_has_piece", note_detection.Get());
    intake_esc_.Write(frc846::control::ControlMode::Velocity, intake_speed_.value() +
       frc846::util::ShareTables::GetDouble("velocity") * 1_tps / (intake_diameter_.value().to<double>() * 3.1415926 / 12.0));
    ...
    frc846::util::ShareTables::SetBoolean("intake_has_piece", false);

    intake_esc_.Write(frc846::control::ControlMode::Velocity, intake_feed_speed_.value());

  } else if (target.target_state == kRelease){
    intake_esc_.Write(frc846::control::ControlMode::Percent, release_speed_.value());

  } else if (target.target_state == kLodge){
    note_detection.EnableLimitSwitch(false);
    
    intake_esc_.Write(frc846::control::ControlMode::Velocity, intake_speed_.value() * lodge_speed_.value());

  } else if (target.target_state == kRollerIn){
    note_detection.EnableLimitSwitch(false);
    
    intake_esc_.Write(frc846::control::ControlMode::Velocity, intake_speed_.value() * roller_in_speed_.value());

  } else if (target.target_state==kIntakeIdle){
    intake_esc_.Write(frc846::control::ControlMode::Percent, 0.0);
