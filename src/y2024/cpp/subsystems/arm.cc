...
ArmSubsystem::ArmSubsystem(bool init)
    : frc846::Subsystem<ArmReadings, ArmTarget>{"arm", init} {
    if (init) {
...
ArmTarget ArmSubsystem::ZeroTarget() const {
  ArmTarget target;
...
ArmTarget ArmSubsystem::MakeTarget(
...
bool ArmSubsystem::VerifyHardware() {
  if (is_initialized()) {
    bool ok = true;

    FRC846_VERIFY(tele_one_.VerifyConnected(), ok, "Telescope not connected");
...
ArmReadings ArmSubsystem::GetNewReadings() {
...
void ArmSubsystem::PositionTelescope(ArmTarget target) {
...
void ArmSubsystem::PositionPivot(ArmTarget target) {
...
void ArmSubsystem::PositionWrist(ArmTarget target) {
...
void ArmSubsystem::DirectWrite(ArmTarget target) {
