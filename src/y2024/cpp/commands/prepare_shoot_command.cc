...
void PrepareShootCommand::Execute() {
  ...
  pivot_.SetTarget(pivot_.MakeTarget(units::degree_t(setpoints::kAutoShoot(0))));
  ...
  wrist_.SetTarget(wrist_.MakeTarget(units::degree_t(setpoints::kAutoShoot(2))));//110_deg - wrist_.wrist_home_offset_.value() + theta));
