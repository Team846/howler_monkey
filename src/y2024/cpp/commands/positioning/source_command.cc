...
SourceCommand::SourceCommand(
    ...
    : frc846::Loggable{"source_command"}, pivot_(container.pivot_),
...
void SourceCommand::Initialize() {
  Log("Source Command Initialize");
}

void SourceCommand::Execute() {

  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "source");
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="source"){
    units::degree_t nextPivotTarget = units::degree_t(setpoints::kSource(0));
    units::inch_t nextTelescopeTarget = units::inch_t(setpoints::kSource(1));
    units::degree_t nextWristTarget = units::degree_t(setpoints::kSource(2));
    ...
    if (pivot_.readings().pivot_position>5.0_deg && telescope_.readings().extension<1.0_in){
