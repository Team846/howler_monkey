...
PreClimbCommand::PreClimbCommand(
    RobotContainer& container)
    : frc846::Loggable{"pre_climb_command"}, pivot_(container.pivot_),
  ...
  SetName("pre_climb_command");
}

void PreClimbCommand::Initialize() {
  Log("PreClimb Command Initialize");
}

void PreClimbCommand::Execute() {

  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "pre_climb");
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="pre_climb"){
    units::degree_t nextPivotTarget = units::degree_t(setpoints::kPreClimb(0));
    units::inch_t nextTelescopeTarget = units::inch_t(setpoints::kPreClimb(1));
    units::degree_t nextWristTarget = units::degree_t(setpoints::kPreClimb(2));
...
void PreClimbCommand::End(bool interrupted) {
  Log("PreClimb Command Finished");
}

bool PreClimbCommand::IsFinished() {
  return false;
}