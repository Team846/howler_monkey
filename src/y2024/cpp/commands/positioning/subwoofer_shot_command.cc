...
SubwooferShotCommand::SubwooferShotCommand(
    RobotContainer& container)
    : frc846::Loggable{"subwoofer_shot_command"}, pivot_(container.pivot_),
  ...
  SetName("subwoofer_shot_command");
}

void SubwooferShotCommand::Initialize() {
  Log("SubwooferShot Command Initialize");
}

void SubwooferShotCommand::Execute() {

  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "subwoofer_shot");
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="subwoofer_shot"){
    ...
    units::degree_t nextWristTarget = units::degree_t(setpoints::kShoot(2));
    ...
    if (telescope_.readings().extension<1_in){
...
void SubwooferShotCommand::End(bool interrupted) {
  Log("SubwooferShot Command Finished");
}

bool SubwooferShotCommand::IsFinished() {
  return false;
}