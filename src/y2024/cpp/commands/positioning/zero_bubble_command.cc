...
ZeroBubbleCommand::ZeroBubbleCommand(
    RobotContainer& container)
    : frc846::Loggable{"zero_bubble_command"}, pivot_(container.pivot_),
  ...
  SetName("zero_bubble_command");
}

void ZeroBubbleCommand::Initialize() {
  Log("ZeroBubble Command Initialize");
}

void ZeroBubbleCommand::Execute() {

  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "zero_bubble");
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="zero_bubble"){
    units::degree_t nextPivotTarget = units::degree_t(setpoints::kZeroBubble(0));
    units::inch_t nextTelescopeTarget = units::inch_t(setpoints::kZeroBubble(1));
    units::degree_t nextWristTarget = units::degree_t(setpoints::kZeroBubble(2));
...
void ZeroBubbleCommand::End(bool interrupted) {
  Log("ZeroBubble Command Finished");
}

bool ZeroBubbleCommand::IsFinished() {
  return false;
}