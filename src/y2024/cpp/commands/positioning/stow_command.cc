...
StowCommand::StowCommand(
    ...
    : frc846::Loggable{"stow_command"}, pivot_(container.pivot_),
...
void StowCommand::Execute() {
  // std::cout<<"in stow"<<std::endl;z
  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "stow");
    // std::cout<<"set to stow"<<std::endl;
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="stow"){
    // std::cout<<"running stow"<<std::endl;
    units::degree_t nextPivotTarget = units::degree_t(setpoints::kStow(0));
    units::inch_t nextTelescopeTarget = units::inch_t(setpoints::kStow(1));
    units::degree_t nextWristTarget = units::degree_t(setpoints::kStow(2));
