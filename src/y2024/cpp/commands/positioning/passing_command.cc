...
PassingCommand::PassingCommand(
    RobotContainer& container)
    : frc846::Loggable{"passing_command"}, pivot_(container.pivot_),
  ...
  SetName("passing_command");
}

void PassingCommand::Initialize() {
  Log("Passing Command Initialize");
}

void PassingCommand::Execute() {

  if (frc846::util::ShareTables::GetString("arm_position")==""){
    frc846::util::ShareTables::SetString("arm_position", "passing");
  }
  if (frc846::util::ShareTables::GetString("arm_position")=="passing"){
...
void PassingCommand::End(bool interrupted) {
  Log("Passing Command Finished");
}

bool PassingCommand::IsFinished() {
  return false;
}