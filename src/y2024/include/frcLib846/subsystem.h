#ifndef frcLib846_SUBSYSTEM_H_
#define frcLib846_SUBSYSTEM_H_
...
#define frcLib846_VERIFY(expr, ok, fail_msg)    \
  do {                                       \
    if (!(expr)) {                           \
      ok = false;                            \
...
class Subsystem : public frc2::SubsystemBase, public SubsystemBase {
 ...
 public:
  ...
  explicit Subsystem(std::string name, bool init = false) : frcLib846::SubsystemBase{name}, init_{init} {
    if (init_) Init();
  }

  // Construct a subsystem as a child of another subsystem.
  Subsystem(const Loggable& parent, std::string name, bool init = false)
      : frcLib846::SubsystemBase{parent, name}, init_{init} {
    if (init_) Init();
  }
