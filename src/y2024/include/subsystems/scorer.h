...
class ScorerSubsystem : public frc846::Subsystem<ScorerReadings, ScorerTarget> {
 public:
  ScorerSubsystem(bool init);

  ScorerTarget ZeroTarget() const override;
  ScorerTarget MakeTarget(ScorerState target_state, bool opr = false);
  ...
  frc846::Pref<units::turns_per_second_t> intake_feed_speed_{
      *this, "intake_feed_speed_v_", 50_tps};
 ...
 private:
  ...
  frc846::control::SparkRevController<units::turn_t> intake_shooter_esc_{
      *this, "intake_shooter_esc_", ports::scorer_::kController_CANID};
